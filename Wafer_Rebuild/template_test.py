...
class template_match_tool():
    ...
    def template_match_prys(self,image:np.ndarray,template:np.ndarray,template_standard:float):
        ...
        roi = (2*self.template_width,2*self.template_hight)
        best_angle = 0
        src_pry = cv.pyrUp(cv.pyrUp(self.src_image[self.src_image_center[1]-roi[1]:self.src_image_center[1]+roi[1],
                                                   self.src_image_center[0]-roi[0]:self.src_image_center[0]+roi[0]])) # 放大X2
        template_pry = cv.pyrUp(cv.pyrUp(self.template))
        ...
        for i in range(pry_layers_num+1,0,-1):
            ...
            if i == pry_layers_num+2:
                angle_range = [-6,6]
                angle = angle_range[0]
            else:
                angle_range = [best_angle-3*min_step-0.001,best_angle+3*min_step+0.001]
                angle = angle_range[0]
            print(angle_range,best_angle)
            score_list = []  # 添加score_list定义
            #print(angle_range)
            while angle<=angle_range[1] and angle>=angle_range[0]:      
                ...
                cv.imshow("template",template_rotate)
                cv.waitKey(0)
                ret,ret_content,BM = self.template_match(src_image,template_rotate,0.6)
                score_list.append([BM[2],BM[0],BM[1],angle])  # 修正添加元素的方式
                angle = angle+min_step
            # 按照数组的第一位元素排序(从大到小)
            score_list.sort(key=lambda x: x[0], reverse=True)
            best_angle = score_list[0][3]
            best_match = score_list[0][1:]
            result_image = cv.cvtColor(self.src_image,cv.COLOR_GRAY2BGR)
            cv.circle(result_image, (best_match[0], best_match[1]), 5, (0, 0, 255), -1)
    
            for score in score_list:
                print(score[0],score[3])
        return 0,0,best_match,result_image
...
if __name__ == "__main__":
    ...
    for i in os.listdir("Data_tempalte_match"):
        ...
        if ret:
            ret,ret_1,image_result,result_image = T_M_T.template_match_prys(img,template,0.8)
            print(i,image_result[2])
            
            cv.circle(img,(image_result[0],image_result[1]),10,(0,0,255),-1)
