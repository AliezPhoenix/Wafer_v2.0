...
class WorkThread(QThread):
    ...
    def __init__(self, **kwargs):
        ...
        self.web_frame_queue    = kwargs.get('web_frame_queue', queue.Queue(maxsize=10))
        self.web_display_enabled = kwargs.get('web_display_enabled', False)
        ...
        try:
            ...
            # 但为了更安全，分别处理每个相机，避免一个相机失败影响其他相机
            cameras = [
                (self.Camhigh, "高倍相机1"),
                (self.Camhigh2, "高倍相机2"),
                (self.Camlow, "低倍相机1"),
            ...
            for cam, name in cameras:
                if cam is not None:
                    try:
                        ...
                        if self.logger:
                            self.logger.warning(f"关闭{name}时出错: {e}")
        except Exception as e:
            # 外层异常处理，防止初始化阶段出错导致线程无法启动
            if self.logger:
                self.logger.warning(f"关闭相机时发生异常: {e}")
            pass
    ...
    def actions(self,action:str):
        ...
        cut_path_widthMax_std   = signals_current.decode("DT1032","DT1032","int")
        cut_path_widthMin_std   = signals_current.decode("DT1033","DT1033","int")
        cut_path_Twidth_std     = signals_current.decode("DT1034","DT1034","int")
        ...
        product_type            = signals_current.decode("DT1044","DT1044","int")
        ...
        if action == "Template_Match":
            ...
            #### 芯片尺寸的1.5倍作为roi
            ##chip_size(0.1um)  scale_x = 4 scale_y = 2
            roi_x = int(chip_size[0]/100)*4  
            roi_y = int(chip_size[1]/100)*2
            
            if (roi_x <max(template_width,template_hight)*4) or (roi_y <max(template_width,template_hight)*2):
                roi_x = max(template_width,template_hight)*4
                roi_y = max(template_width,template_hight)*2
            if roi_x >600 or roi_y>450 or product_type == 1:
                roi_x = 600
                roi_y = 450
            #roi_x = template_width*2
            #roi_y = template_hight
            #### ret = 0 模板匹配成功
            #### ret = 1 模板匹配失败
            #### ret = 2 Q值未达到设定值
            if current_cam == 0 or current_cam == 3:
                ret_multiple,ret_content,result_template = template_match_multiple(image,template,template_standard,(roi_x,roi_y))
                print("ret_multiple: ",ret_multiple,"  ret_content: "+ret_content)
                if ret_multiple != 0 :
                    log.warning("模板匹配多目标角度计算失败,采用dll计算")
                    ...
                    if angel <0.7 and angel >-0.7 :
                        ret,ret_content,closest_template = template_match(image,template,template_standard)
                        
                        if closest_template[2] >= result_template[2]-0.005:
                            angel = 0
                        if closest_template[2] >= result_template[2]-0.01:
                            angel = angel/2
                else:
                    ret = ret_multiple
                    angel = result_template[3]
            ...
            if ret != 0 :
                ...
                cv.imwrite("Data_failed\\template_match\\failed{}.bmp".format(time.time()),image)
                ...
                cv.rectangle(image,(image_center[0]-roi_x,image_center[1]-roi_y)
                             ,(image_center[0]+roi_x,image_center[1]+roi_y),(255,0,255),2)
    ...
    def display_image(self,temp,border_on_or_off,border_color):
        ...
        image_pix_map = None
        image_animation = None  # 初始化变量以便在方法作用域内使用
        ...
        if mode == "live":
            ...
            if self.map_data == []:
                ...
                bytes_per_line = self.image_show.shape[2]*self.image_show.shape[1]
                image_pix_map = QImage(self.image_show.data, self.image_show.shape[1], self.image_show.shape[0],bytes_per_line,QImage.Format_BGR888)
                image_animation = self.image_show  # 设置为 image_show 以便 web 显示使用
            else:
                current_path_sp = [0,0]
        ...
        if image_pix_map is not None:
            try:
                # 检查 QLabel 是否仍然有效
                if self.image_show_lable is not None:
                    # 尝试访问对象的属性来检查是否已被删除
                    try:
                        ...
                        log.error(f"更新图像显示时发生错误: {e}")
            except Exception as e:
                log.error(f"检查 QLabel 时发生错误: {e}")

        if self.web_display_enabled:
            try:
                if mode == "live":
                    if temp:
                    ...
                    # 在 animation 模式下，使用 image_animation（已在上面定义）
                    if image_animation is not None:
                        display_image = image_animation.copy()
                    else:
                        display_image = self.image_show.copy()
                else:
                    # 其他模式使用 image_show
                ...
                if ret:
                    self.web_frame_queue.put(jpeg.tobytes())
                    # 将帧数据放入队列供Web显示使用
                    if self.web_frame_queue.full():
                        try:
                            self.web_frame_queue.get_nowait()  # 移除旧帧
                        except queue.Empty:
                            pass
                
            except Exception as e:
                log.error(f"网页同步显示错误: {e}")
        
        # 返回JPEG数据（保持原有功能）
        # if mode == "live":
        #     if temp:
        #         ret, jpeg = cv.imencode('.jpg', self.image)
        #     else:
        #         ret, jpeg = cv.imencode('.jpg', self.image_show)
        # else:
        #     ret, jpeg = cv.imencode('.jpg', image_animation)
        # return jpeg.tobytes()
    
    def run(self):          
        connection_ret = self.COM.connect()
        log = self.logger
        if connection_ret != 0:
            log.error("COM连接失败，退出线程")
