...
if __name__ == "__main__":
    path                    = os.path.join(os.curdir,"Wafer_Rebuild\Dll\Template_Match_Dll.dll")
    ...
    for i in os.listdir("Wafer_Rebuild\Data_template_match3"):
        print(i)
        image = cv.imread("Wafer_Rebuild\Data_template_match3\\"+i)
        image_width             = image.shape[1]
        image_hight             = image.shape[0]
        image_center            = [int(image_width/2),int(image_hight/2)]
        template = cv.imread("Wafer_Rebuild\Data_template_match3\Template_1.bmp")
        template_width = template.shape[1]
        template_hight = template.shape[0]
        template_max = max(template_width,template_hight)+max(template_width,template_hight)
        #kernel = cv.getRotationMatrix2D((image_width/2,image_hight/2),5,1)
        #image = cv.warpAffine(image,kernel,(image_width,image_hight))
        cv.imshow("image",image)
        cv.waitKey(0)
        start_time = time.time()
        ret,ret_1,image_result = Functions.template_match_multiple(image,template,80,(600,200))
        end_time = time.time()
        print(f"时间: {end_time - start_time}秒")
        print(ret,ret_1,image_result)
        #ret,ret_1,image_result = Functions.template_dll_match(image,template,600,450,dll,90)
        print(image_result)
        if ret == 0:
